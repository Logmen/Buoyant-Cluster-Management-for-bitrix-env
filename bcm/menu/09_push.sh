...
_push_show_status() {
    ...
    for node in "${BCM_NODES_WEB[@]}"; do
        ...
        # ⚠️⚠️ `grep -c` печатает «0» при нуле совпадений, но ВЫХОДИТ rc=1. На ноде без
        # клиентов (напр. свежий BACKUP web02 после reboot) ssh возвращает этот rc=1, а
        # под `set -o pipefail` он ПРОСАЧИВАЕТСЯ через `… | tr` (tr=0, но pipefail берёт
        # ненулевой) → присваивание clients=$(…) даёт rc=1 → `set -e` УБИВАЛ bcm прямо
        # посреди таблицы (печаталась строка web01 с клиентами, web02 с 0 → вылет в шелл;
        # ловили вживую). `|| true` на УДАЛЁННОЙ стороне делает rc=0, «0» из grep остаётся.
        clients=$(bcm_ssh_exec_timeout "$ip" 8 \
            "ss -tn state established 2>/dev/null | grep -cE ':(801[0-9]|901[0-9])\\b' || true" \
            2>/dev/null | tr -d '[:space:]')
        [[ "$clients" =~ ^[0-9]+$ ]] || clients="?"
