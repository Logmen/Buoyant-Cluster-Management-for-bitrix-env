...
_cn_single_node_mode() {
    ...
    bcm_color "DIM" "  HAProxy) и направляет чтения БД на writer. lsyncd ЗАМОРАЖИВАЕТСЯ на"
    bcm_color "DIM" "  всех web-нодах: тёплые ноды держатся как известно-целая копия кода"
    bcm_color "DIM" "  и не получают правок с активной, пока режим включён."
    if ! bcm_s3_enabled; then
        bcm_color "DIM" "  Зеркало /upload при этом продолжает работать: оно только добавляет"
        bcm_color "DIM" "  файлы на пиры и ничего не затирает."
    fi
    bcm_color "DIM" "  Удобно для первичной заливки или переноса портала, чтобы трафик и"
    bcm_color "DIM" "  запись кода не «гуляли» между нодами."
    echo
