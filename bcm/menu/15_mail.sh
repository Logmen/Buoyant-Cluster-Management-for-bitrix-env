...
_mail_dns_hints() {
    ...
    echo
    echo "  Поскольку почта уходит ТОЛЬКО через релей (${relay:-внешний SMTP}),"
    echo "  выравнивайте записи под него (точные значения — у провайдера релея):"
    echo
    echo "  • SPF (TXT @ ${dom}):"
    echo "      v=spf1 include:<spf-релея> -all"
    echo "      (например include:_spf.<релей>; НЕ перечисляйте IP web-нод — они не шлют наружу)"
    echo
