...
# Пояснение к статусу. ⚠️ Раньше здесь висело безусловное «lsyncd должен быть
# остановлен», написанное для этапа до деплоя портала. На работающем кластере это
# вводит в заблуждение: lsyncd штатно запущен на источнике и штатно остановлен на
# остальных нодах, роль им управляет автоматически. Предупреждение оставлено только
# для случая, когда портал ещё не развёрнут.
_warn_portal_required() {
    echo
    if [[ -z "$(bcm_get_active_node 2>/dev/null || echo '')" ]]; then
        bcm_color "YELLOW_BOLD" "  ╔══════════════════════════════════════════════════════════════╗"
        bcm_color "YELLOW_BOLD" "  ║  ⚠  Источник синхронизации не назначен.                      ║"
        bcm_color "YELLOW_BOLD" "  ║  До деплоя портала lsyncd должен быть остановлен на всех      ║"
        bcm_color "YELLOW_BOLD" "  ║  узлах — иначе пустое дерево уедет на остальные.              ║"
        bcm_color "YELLOW_BOLD" "  ╚══════════════════════════════════════════════════════════════╝"
    else
        bcm_info "Источник — держатель web-VRRP; на нём lsyncd активен, на остальных узлах"
        bcm_info "остановлен. Ролью управляет keepalived, вручную запускать не нужно."
    fi
    echo
}
...
-- Контент /upload — в S3/MinIO (catch-all FILE_RULES), локально его нет →
-- односторонний блок БЕЗ --delete безопасен (анти-клоббер); resize_cache/tmp
-- исключены (per-node, регенерируются).
sync {
    default.rsync,
    source = "${site_path}/upload/",
    target = "root@${dst_ip}:${site_path}/upload/",
    rsync = {
        archive  = true,
        compress = true,
        rsh      = "ssh -o StrictHostKeyChecking=accept-new -o BatchMode=yes -i ${ssh_key}",
