...
_mod_table() {
    ...
    # ⚠️ Явный return 0: последним в функции стоит условие, и когда показывать
    # нечего, оно возвращает 1. Под set -e это роняло весь раздел, а bin/bcm
    # (тоже set -e) следом закрывал консоль вместо возврата в главное меню.
    return 0
}
