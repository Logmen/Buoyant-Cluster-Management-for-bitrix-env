...
# Маркер контролируемого переезда — см. redis_session_check.sh. Проверяется ПЕРВЫМ,
# до ветки «не держим VIP → здоровы», иначе держатель не ушёл бы в FAULT.
[[ -e /run/bcm-vrrp-fault-transformer ]] && exit 1
