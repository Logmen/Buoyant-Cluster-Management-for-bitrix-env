...
# ──── Отфильтровать пустые элементы списка ───────────────────────────────────
# ⚠️ "${arr[@]:-}" на ПУСТОМ массиве раскрывается в ОДИН элемент — пустую строку,
# поэтому проверка «${#a[@]} -gt 0» считает отсутствующий слой присутствующим:
# строк не печатается, а разделитель таблицы — да (лишняя пустая полоса на месте
# слоя S3, которого в кластере нет). Печатает только непустые элементы, по одному
# на строку — под mapfile.
_bcm_nonempty() {
    local v
    for v in "$@"; do
        [[ -n "$v" ]] && printf '%s\n' "$v"
