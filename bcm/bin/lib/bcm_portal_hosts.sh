...
#
# ⚠️ Записи ставятся ПЕРЕД '# ANSIBLE MANAGED BLOCK': ansible bitrix-env
# перезаписывает свой блок, а при некоторых операциях — и весь /etc/hosts
# целиком (ловили вживую: на обеих web-нодах боевого кластера файл оказался
# сброшен к дистрибутивному виду, обе записи исчезли; обнаружилось только при
# разборе аварии). Поэтому одной установки мало — режим assert переприменяет
# записи и дёргается guard'ом /etc/cron.d/bcm-portal-hosts-guard.
#
# Режимы:
