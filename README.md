...
> Без S3: `/upload` остаётся на дисках web-нод (lsyncd раздаёт `/upload` только
> **с источника** и без `--delete`, поэтому файл, загруженный пользователем на
> НЕ-источник, остальным нодам не виден → 404 при round-robin), а резервное
> копирование не настраивается — хранить копии негде. Пункты меню 11 и 13 в таком
> кластере сообщают, что слой не развёрнут. Разумные варианты без S3: держать портал
> в режиме единой активной ноды (меню 1 → 7) и делать копии внешними средствами.
