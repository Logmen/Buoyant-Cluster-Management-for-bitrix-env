...
> настраивается — хранить копии негде; пункты меню 11 и 13 сообщают, что слой не
> развёрнут. Зеркало — репликация, а не бэкап: удаления не зеркалятся, а файл,
> принятый за секунды до отказа ноды, доехать до пиров не успевает.
...
* **`/upload`** (пользовательские файлы, пишутся на любой ноде) → выносятся в общий
  **MinIO S3** через модуль Bitrix «Облачные хранилища». ⚠️ Слой S3 опционален: если он
  не развёрнут, этот пункт не работает — файл остаётся на диске принявшей его ноды и
  другим нодам не виден (см. «Состав кластера»). Установщик создаёт бакет
...
* **`/upload` в кластере БЕЗ S3** → **зеркало между всеми web-нодами**
  ...
  записи). Цена: удаления не зеркалятся — на пирах остаются сироты. При добавлении
  слоя S3 зеркало снимается автоматически, а `/upload` уходит в бакет.
