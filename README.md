...
[Слой 3]   Веб-ноды — Nginx · Apache · PHP · bx-push-server (NodeJS RTC) ← МОЗГ
   ...
   ▼
[Слой 4]   ProxySQL :6033 (embedded в web-ноды) — записи и служебное → writer (HG10);
   │       чистые SELECT портала → реплики (HG20) через класс подключения BCM (разделение чтений)
   │
   ▼
...
| **S3** | `s3` | 2 или 0 | MinIO (Site Replication Active-Active) — **слой опционален** |

> **S3 — единственный опциональный слой.** Допустимо 0 нод (слой выключен) либо 2 и
> более; одна нода отвергается — всё хранилище файлов портала осталось бы без резерва.
> Без S3 `/upload` остаётся на дисках web-нод, поэтому установщик поднимает **зеркало
> `/upload`** — отдельный always-on lsyncd на каждой web-ноде, раздающий принятые ею
> файлы всем пирам (см. «Разделение данных»). Резервное копирование при этом не
> настраивается — хранить копии негде; меню 13 сообщает, что цели нет, а меню 11
> предлагает подключить **внешний бакет** провайдера (для облачного `/upload` свои
> S3-ноды не нужны). Зеркало — репликация, а не бэкап: удаления не зеркалятся, а файл,
> принятый за секунды до отказа ноды, доехать до пиров не успевает. Появилось
> хранилище — зеркало по умолчанию снимается, но его можно (и часто нужно) оставить:
> `[lsyncd] upload_mirror = on`, меню **6 → 10**.
...
* Список IP и имён серверов для каждого слоя (`lb`, `web`, `pxc`, `s3`); для `s3`
  допустимо ответить `0` — кластер развернётся без объектного хранилища.
...
PXC_WRITER="db01"
# Слой S3 опционален: S3_NODES="" — без объектного хранилища (тогда остальные S3_*
# игнорируются); иначе 2 и более нод.
...
# Резервное копирование (HA-aware). Ключ шифрования генерируется install'ом.
# BACKUP_TARGET: s3 (бакет MinIO кластера) или nfs (внешний каталог, слой S3 не нужен).
BACKUP_TARGET="s3"
BACKUP_BUCKET="bitrix-backups"; BACKUP_RETENTION_DAYS="14"
# Для BACKUP_TARGET="nfs": экспорт внешнего хранилища. NFS_SUBDIR — подкаталог под
# этот кластер, если экспорт общий для нескольких.
NFS_SERVER="";  NFS_EXPORT="";  NFS_MOUNT="/mnt/bcm-backup";  NFS_SUBDIR=""
...
* **БД** — на всех `web` правило ProxySQL `^SELECT` (rule_id=4) направляется в
  `HG_WRITE`, а пользователь-читатель разделения чтений (если оно включено) на время
  режима переводится на `HG_WRITE`: все запросы портала идут на одного writer'а.
  Выключение режима возвращает читателя на реплики.
...
## Подключаемые модули (меню 16)

Ядро управляет кластером; всё, что растёт вокруг — веб-портал, экспортёры,
интеграции, — живёт в **модулях** и не требует ни форка BCM, ни патчей к
...
Модуль может разрабатываться **в своём репозитории и поставляться отдельно от
BCM**, в том числе коммерчески: он пишется под версию контракта (`MODULE_API`), а
не под версию ядра, ставится из `tar.gz` и проверяется перед установкой —
`.sha256` сверяется обязательно, подпись `.asc` — ключами поставщиков из
`/etc/bitrix-cluster/module-keys/` (подписанный пакет без доверенного ключа не
ставится). Кто выпустил модуль и на каких условиях, оператор видит в меню.

Контракт для авторов модулей — [docs/modules.md](docs/modules.md). Первый модуль
по нему — **BCM Portal** (отдельный продукт: скрипты копий на узлах базы и фронт
HAProxy на балансировщиках для веб-портала).
...
учётки `bitrix`/`bitrix_ro`/`monitor` создавал с `host='%'`. На кластере с публичными
адресами это означало **3306 и 4567 доступны из интернета** — от базы (у `bitrix`
`ALL PRIVILEGES`) отделял один пароль. Плюс `root@localhost` с пустым паролем: только
сокет, но любой shell на ноде = root базы.

Теперь (`bin/lib/bcm_pxc_harden.sh`; ставит `install.sh`, для живого кластера —
меню **3 → 11**, шаги идемпотентны):

* **firewall PXC**: 3306 — только с web-нод (ProxySQL) и PXC-пиров, 4567/4568/4444 —
  только с PXC-пиров; `service mysql` и открытые порты снимаются. Разрешения ставятся
  **до** снятия широких правил — репликация и ProxySQL не рвутся;
* **учётки** получают маску подсети кластера (`a.b.c.%`, все ноды в одной /24; иначе
  `'%'` с предупреждением). На старом кластере — `RENAME USER`: пароль и гранты целы,
  живые соединения не трогаются;
* **`root@localhost` → `auth_socket`**: тот же доступ root'у ОС по сокету, никому
  больше. Плагин ставится на **каждой** ноде (`INSTALL PLUGIN` Galera не реплицирует),
  затем `ALTER USER` на writer (TOI). Инструменты BCM ходят как root ОС по сокету
  (`mysql` без `-h`, `xtrabackup` через `[client] socket`) — им ничего не меняется.
  Откат: `ALTER USER 'root'@'localhost' IDENTIFIED WITH caching_sha2_password BY ''`.
...
* **`/upload`** (пользовательские файлы, пишутся на любой ноде) → выносятся в
  **S3** через модуль Bitrix «Облачные хранилища»: либо свой слой MinIO, либо **внешний
  бакет провайдера** (меню **11 → «Подключить внешнее S3-хранилище»**,
  `bin/lib/bcm_s3_external.sh` — проверяет доступ и пишет `[s3_upload] provider = external`).
  ⚠️ Хранилище опционально: без него файл остаётся на диске принявшей его ноды и другим
  нодам не виден (см. «Состав кластера») — тогда его зеркалит `lsyncd-upload`.
  ⚠️ **Стиль адресации бакета задаёт версия модуля**: `clouds` ≥ **26.100** строит и
  запрос, и ссылку как **path-style** (`https://<host>/<bucket>/<key>`), более старые —
  только virtual-hosted-style (`https://<bucket>.<host>/<key>`, нужен wildcard-DNS и
  сертификат под него). Проверка при подключении внешнего бакета определяет, какие стили
  держит провайдер, сверяет с версией модуля на ноде и пишет итог в `[s3_upload] addressing`. Установщик создаёт бакет
  ...
  ⚠️ **После переноса файлов в облако** Битрикс удаляет локальные копии на той ноде,
  где шёл перенос, а ссылки на **физический путь** `/upload/…` остаются в мире: код
  виджета «Кнопка на сайт» на сайтах клиентов, трекер звонков, старые письма. Такой
  запрос на ноду без копии получал HTML-404, и браузер блокировал его как `<script>`
  (ORB) — виджет пропадал у половины посетителей. Меню **11 → 5** ставит на web-ноды
  nginx-фрагмент (`bx/site_settings/default/bcm_s3_upload.conf`): файл есть на диске —
  отдаётся с диска, нет — проксируется из бакета тем же стилем адресации. Защитные
  правила bitrix-env для `/upload` (svg, исполняемые расширения, `1c_`, `resize_cache`)
  фрагмент не перекрывает. Зеркало `lsyncd-upload` тут не помощник: удаления оно не
  переносит намеренно, а перенос в облако — это и есть удаление.
...
* **зеркало `/upload` между всеми web-нодами**
  (`bin/lib/lsyncd_upload.sh`, юнит `lsyncd-upload`, меню **6 → 10**). Основной lsyncd
  тут не помогает: он односторонний и работает только на держателе web-VRRP, а
  загрузки приходят на любую ноду. Отдельный always-on инстанс на каждой ноде шлёт
  свой `/upload` пирам с `delete=false` и `--update` (встречные потоки с `--delete`
  затирали бы друг друга; коллизий имён нет — Bitrix пишет по уникальному пути от ID
  записи). Цена: удаления не зеркалятся — на пирах остаются сироты.

  Нужно ли зеркало — решает **`[lsyncd] upload_mirror` в `cluster.conf`**
  (`auto` | `on` | `off`, единый предикат `bcm_upload_mirror_wanted`):

  | режим | смысл |
  |---|---|
  | `auto` (умолчание) | зеркало держим, пока для `/upload` нет хранилища; появилось (свой слой S3 или внешний бакет) — снимается |
  | `on` | зеркало держим **всегда, в том числе вместе с S3** |
  | `off` | зеркала нет |

  ⚠️ **`on` — не избыточность, а норма для существующих порталов.** В бакет уходит
  только то, что попало под `FILE_RULES`; статика модулей, `resize_cache` и файлы,
  залитые до подключения хранилища, остаются на дисках — и обязаны быть на **всех**
  нодах, иначе round-robin отдаёт по ним 404. Полностью «пустой» локальный `/upload`
  бывает лишь при catch-all-правилах и выполненном переносе старых файлов в облако.

  ⚠️ Зеркало включено ⇒ **основной lsyncd блок `/upload` не генерирует** (одно дерево
  не должны толкать два инстанса) — это учитывают меню 6 → 3, `lsyncd_role.sh`
  (перечитывает режим при каждом `promote`) и `install.sh`. Режим переживает повторный
  `install.sh`: `write_conf` переносит его в новый конфиг вместе с внешним `[s3_upload]`.
...
## Фоновая шина Messenger ядра: с хитов на master-cron (меню 10 → 10)

Ядро Bitrix (main 26.x) в режиме `run_mode=web` (умолчание, секции `messenger` в
`.settings.php` нет) на **каждом хите** обходит все очереди внутренней шины
(main, calendar, bizproc, tasks…, на портале их больше тридцати), и каждая берёт
именованную блокировку `GET_LOCK` с нулевым таймаутом. На нагруженном портале это
четыре пятых всех запросов к БД и спам варнингов PXC про `GET_LOCK` в `error.log`.

Штатный выход ядра — `run_mode=cli`: хиты очереди не трогают, их разбирает
потребитель. Консольная команда `messenger:consume` здесь непригодна (консоль
`bitrix.php` требует Symfony Console, которого в bitrix-env нет), поэтому BCM ставит
свой потребитель `local/cron/bcm-messenger.php` (тот же цикл `Worker::process()` до
лимита времени) и запускает его раз в минуту из класса кронов «только master»
(`/etc/cron.d/bcm-portal-master`, переезжает вместе с web-VRRP, повторный запуск в ту
же минуту отсекает `flock`). Секция `messenger` в `.settings.php` берётся под защиту
сторожа настроек. Пауза между проходами — 5 с: проход по всем очередям стоит около
полусотни запросов, так потребитель даёт единицы процентов прежней нагрузки при
задержке фоновых задач до пяти секунд. Журнал — `journalctl -t bcm-messenger`,
статус (режим, потребитель, глубина очередей) — пункт «статус» того же меню.
Исполнитель на web — `bin/lib/bcm_messenger.sh`.
...
## Разделение чтений: SELECT портала на репликах PXC (меню 4 → 7)

Штатно ProxySQL в BCM — HA-прокси: правило `^SELECT` (rule_id=4) ведёт на writer, и
две реплики PXC простаивают. Причина в том, что одно stateless-правило не умеет
«не уводить SELECT, пока соединение держит `GET_LOCK`»: залоченный именованной
блокировкой коннект ProxySQL отказывается двигать на другую hostgroup (ошибка 9006 при
обновлении модулей). Штатное решение Bitrix — модуль «Веб-кластер» — доступен не во
всех редакциях и рассчитан на классическую репликацию.

BCM даёт **свой класс подключения** `Bcm\DbRouter\Connection`
(`/local/modules/bcm.dbrouter`, наследник штатного `MysqliConnection`), который
подставляется в `bitrix/.settings.php` через документированный параметр `className`
подключения. Ядро не правится, редакция не важна. Класс держит второе соединение к
ProxySQL под **пользователем-читателем** `<db_user>_ro` (тот же пароль, в PXC только
`SELECT`, `default_hostgroup=HG_READ`, правило 4 на него не распространяется) и решает
маршрут для каждого запроса:

* на читателя идут **только чистые SELECT** — без `FOR UPDATE`/`LOCK IN SHARE MODE`,
  функций блокировок, переменных (`@`), `LAST_INSERT_ID`/`FOUND_ROWS`/`ROW_COUNT` и
  прочего, что привязано к соединению;
* внутри транзакции (уровень ядра и сырые `START TRANSACTION`) — только writer;
* SELECT по таблице, **изменённой в этом же хите**, — только writer («прочитай, что
  сам записал»); DDL, временные таблицы, `LOCK TABLES`, `autocommit=0` переводят
  остаток хита на writer;
* сессионные `SET` (`NAMES`, `collation_connection`, `sql_mode`, `time_zone`…)
  повторяются на reader-линке; явное «только master» ядра (`useMasterOnly`, так
  обёрнуты `lock()`/`unlock()`) соблюдается;
* читатели недоступны → прозрачный откат на writer с паузой повторов (APCu/файл);
  **рубильник** `/etc/bitrix-cluster/dbrouter.off` выключает маршрутизацию без правки
  конфигов (перечитывается раз в 5 с, в том числе долгоживущими процессами).

Причинную согласованность между узлами даёт PXC: `wsrep_sync_wait=1` — SELECT на
реплике ждёт применения всех транзакций, закоммиченных до его начала, поэтому запись
через writer видна следующим чтением через реплику даже из другого хита. На writer'е
очередь применения пуста, цена нулевая.

Класс подгружается автозагрузчиком из `bitrix/.settings_extra.php` (его пишет сторож
настроек): extra читается ядром до создания подключения и не перезаписывается ansible
bitrix-env. Если файлов модуля на ноде нет, имя класса привязывается к штатному —
портал работает без разделения чтений, а не падает.

**Старый API** (`$DB->Query`, на нём держится немалая часть модулей) исполняет SQL мимо
класса подключения — напрямую вызовом `mysqli_query` на ресурсе. Его накрывает подкласс
`Bcm\DbRouter\LegacyDatabase` (наследник `CDatabase` с переопределённым `QueryInternal`):
блок `bcm:dbrouter` в действующем `php_interface/init.php` подменяет `$GLOBALS['DB']`
на него сразу после создания ядром. Правила маршрута и состояние хита (изменённые таблицы,
транзакции, сессионные `SET`) у обоих путей общие. Правится тот `init.php`, который ядро
реально грузит (`/local/php_interface/init.php`, если он есть, иначе `/bitrix/...`);
блок снимается при выключении, бэкап `init.php.bcm-bak-dbrouter`.

Включается `install.sh` при `DB_READ_SPLIT="1"` (по умолчанию) и на уже развёрнутом
кластере через **меню 4 → 7 → 2**: пользователь-читатель в PXC и `wsrep_sync_wait=1`,
читатель в ProxySQL на каждой web, файлы модуля и `className`/`reader` в
`.settings.php`, переснятие эталона сторожа, самопроверка из-под ядра (reader-узел
обязан отличаться от writer). Любой сбой откатывает сделанное. Исполнитель на web —
`bin/lib/bcm_dbrouter.sh` (`--status`, `--selftest`, `--kill on|off`), журнал
`/var/log/bcm/dbrouter.log`, счётчики по hostgroup — в статусе меню.
...
* **Балансировщики (`lb`)**:
  ...
  * **Журнал запросов** (`option httplog`: клиент, выбранный бэкенд и сервер, тайминги
    по фазам, код ответа, флаги завершения сессии) включается правилом
    `/etc/rsyslog.d/49-haproxy.conf`. ⚠️ HAProxy работает в chroot, поэтому его
    `log /dev/log` — это сокет **внутри** `/var/lib/haproxy`; rsyslog слушает
    дополнительный сокет там, иначе записи теряются молча. Смены состояния бэкендов
    (`server ... is DOWN/UP`) видны и в journald: `journalctl -t haproxy`.
* **Веб-серверы (`web`)**:
...
**Несколько доменов на одном кластере.** HAProxy держит каталог pem и выбирает сертификат
по SNI, поэтому дополнительные домены (например, отдельный портал поддержки) ставятся тем же
пунктом 12 → 2 и живут только на LB. На web-ноды и в `[ssl] domain` уходит только сертификат,
покрывающий домен портала: nginx web-нод держит один `cert.pem`, а домен портала там смотрит в
`127.0.0.1`, и всё, что ходит в него с самой ноды (self-check'и Битрикса, MCP-коннектор), проверяет
имя в сертификате. Дополнительные домены видны в статусе 12 → 1.

## Резервное копирование (меню 13)

HA-aware бэкапы (исполнитель `bin/lib/bcm_backup.sh`, настройка и раскатка —
`bin/lib/bcm_backup_setup.sh` и меню **13**). Хранилище задаётся в `cluster.conf`
(`[backup] target`, при установке — `BACKUP_TARGET` из файла ответов):

* **`s3`** — бакет объектного хранилища: **свой** MinIO кластера или **бакет
  провайдера**. Бакет копий обязан быть ОТДЕЛЬНЫМ от бакета `/upload`: у копий свои
  retention, versioning и права, а общий бакет держал бы данные портала вместе с их
  же резервом. Ключи и endpoint берутся из `[s3_upload]`, но `[backup] endpoint`,
  `access_key`, `secret_key` их **перекрывают** — это обязательный случай, когда
  провайдер выдаёт scoped-ключ на каждый бакет (политика `s3:*` только на свой);
* **`nfs`** — сетевой каталог внешнего хранилища; **S3 не требуется**, поэтому
  кластер без объектного хранилища тоже получает полноценные копии.

Обе цели настраиваются прямо из меню **13 → 7**, повторный `install.sh` не нужен:
пункт **13 → 8** раскатывает на ноды `mc`, `backup.env` (0600) и таймеры теми же
шагами, что делает установщик. ⚠️ На ЧУЖОМ бакете BCM не создаёт versioning и
lifecycle (scoped-ключ обычно не имеет на это прав) — их настраивает владелец
хранилища, а проверка при подключении предупреждает, если versioning выключен.
⚠️ `[backup] enc_key` генерируется один раз и переживает повторный `install.sh`
(`write_conf` переносит всю секцию): сменив ключ, вы потеряете доступ к уже
лежащим в хранилище conf-архивам.

Типы копий одинаковы для обеих целей:
...
* **db** — PXC через `xtrabackup --stream` с Synced-ноды (гейты: только Synced,
  `wsrep_desync=ON` на время, идемпотентный маркер);
* **files** — **код портала** с текущего источника lsyncd. ⚠️ `/upload` в эту копию
  НЕ входит: предполагается, что он лежит в облачном бакете. Если облака нет ИЛИ
  правила бакета узкие (в облако уходит лишь часть файлов), пользовательские файлы
  остаются на дисках web-нод и требуют **отдельной копии** — зеркало `lsyncd-upload`
  бэкапом не является.

Чем цели отличаются на практике:

| | S3 | NFS |
|---|---|---|
| Ротация старых копий | lifecycle бакета | `--prune` по `RETENTION_DAYS`, отдельный таймер |
| История (защита от `rm -rf`) | versioning бакета | датированные снимки `rsync --link-dest` — неизменившиеся файлы связаны жёсткими ссылками, место занимает только разница |
| Проверка перед записью | доступность бакета | ⚠️ каталог обязан быть **точкой монтирования**, иначе отвалившийся NFS тихо забил бы системный диск ноды |

Таймеры `bcm-backup-{conf,db,files}` на нодах-кандидатах роли (плюс `prune` для NFS —
на одной ноде, хранилище общее); лишние таймеры раскатка снимает сама (сменилась роль
ноды или цель). Настройка хранилища и восстановление — меню **13** (7 — цель, 8 —
раскатка). Offsite-копия (3-2-1) — пункт 13→6.

---

## Свои настройки: базовый конфиг и слой оператора

Базовые конфиги служб BCM **генерирует из шаблонов** и переписывает при каждом
прогоне `install.sh`. Поэтому править их руками бессмысленно — правки исчезнут.
Для ручных настроек у каждой службы есть **отдельный слой, который BCM не трогает
никогда**; его и открывают пункты меню.

| Служба | Слой оператора | Меню | Как действует |
|---|---|---|---|
| PXC / MySQL | `/etc/my.cnf.d/zz-bcm-custom.cnf` | 3 → 9 | переопределяет базовые значения |
| HAProxy | `/etc/haproxy/conf.d/90-custom.cfg` | 5 → 9 | **дополняет** (см. ниже) |
| Redis (3 инстанса) | `/etc/redis/redis-<инстанс>-custom.conf` | 5 → 10 | переопределяет |
| keepalived | `/etc/keepalived/conf.d/90-custom.conf` | 5 → 11 | **дополняет** |
| ProxySQL | `/etc/bitrix-cluster/proxysql-custom.sql` | 4 → 9 | SQL к admin-интерфейсу |
| nginx (web) | `/etc/nginx/bx/settings/zzz-bcm-custom.conf` | 8 → 10 | переопределяет |

Меню везде делает одно и то же: скачивает слой с доступной ноды, открывает в
`$EDITOR`, **проверяет конфиг нативным средством службы** и раскатывает на все ноды
слоя по одной, с бэкапом и откатом при сбое (держатель VIP — последним, реплики
Redis — раньше мастера).

⚠️ **«Дополняет» и «переопределяет» — разные вещи.** HAProxy и keepalived отвергают
повторное объявление уже существующей секции (`duplicate`), поэтому в их слое заводят
**новые** frontend/backend/`vrrp_instance`, а параметры существующих задаются через
`cluster.conf` и шаблон. Redis, MySQL и nginx, наоборот, читают слой последним, и
заданное в нём побеждает.

⚠️ **ProxySQL — особый случай.** `/etc/proxysql.cnf` читается только при первом
старте, дальше источник правды — его собственная база. Правки файла на работающем
узле не действуют вовсе, поэтому слой здесь — SQL. Порядок применения: SQL →
`LOAD ... TO RUNTIME` → проверка запроса через прокси → и только потом `SAVE ... TO
DISK`; при неудаче `LOAD ... FROM DISK` возвращает последнее рабочее состояние.

### Защита настроек портала от перезаписи

`.settings.php` перезаписывает **ansible bitrix-env** (роль web, задача
«create settings file for site» — модуль `template` без `force: no`, то есть кладёт
скелет при каждом прогоне). Портал при этом теряет подключение к БД, кэш и сессии.

Запретить запись нельзя — в этот файл легально пишет сам Bitrix при установке
модулей. Поэтому перезапись сделана **безвредной**: кластерные секции (`connections`,
`cache`, `session`) дублируются в `.settings_extra.php`, который Bitrix накладывает
**поверх** `.settings.php`. Скелет остаётся, но поверх него продолжают действовать
ProxySQL и redis-VIP.

Эталон лежит вне дерева портала (`/etc/bitrix-cluster/settings-cluster.php`), сторож
`bcm_settings_guard.sh` следит за файлами по inotify (`systemd.path`) и восстанавливает
их за секунды. Под той же защитой — файлы nginx в `bx/settings`: их ansible
синхронизирует с `delete: yes` при добавлении web-ноды.
...
* На `web` нодах доступно **полное меню** (web — «мозг» кластера):
  ...
  | 6 | Синхронизация файлов (lsyncd) | 13 | Резервное копирование (S3 / NFS, HA-aware) |
