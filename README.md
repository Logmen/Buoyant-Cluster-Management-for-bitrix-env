...
HA-aware бэкапы (`configure_backup`, исполнитель `bin/lib/bcm_backup.sh`). Хранилище
выбирается параметром `BACKUP_TARGET`:

* **`s3`** — бакет MinIO кластера (нужен слой S3);
* **`nfs`** — сетевой каталог внешнего хранилища; **слой S3 не требуется**, поэтому
  кластер без своего объектного хранилища тоже получает полноценные копии.
...
* **files** — **код портала** с текущего источника lsyncd. ⚠️ `/upload` в эту копию
  НЕ входит: при развёрнутом S3 он лежит в бакете `bitrix-upload`; в кластере без S3
  файлы остаются на дисках web-нод и требуют отдельной копии.
...
на одной ноде, хранилище общее). Настройка хранилища и восстановление — меню **13**,
пункт 13→7 настраивает NFS прямо из интерфейса. Offsite-копия (3-2-1) — пункт 13→6.
